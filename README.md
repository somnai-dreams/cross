...
The included player script and minified CSS total about **91 KB raw / 27 KB gzipped**. A complete ordinary puzzle file is about **114 KB raw**, including the base64 player and both native puzzle copies. Gzip size is a transfer measurement, not the size of a downloaded HTML file. CI checks the bundle stays below 110 KB raw and 30 KB gzipped and that the committed bundle matches its source.
...
| `@somnai-dreams/cross/embed` | `mountPlayer` with the included UI in the host DOM, CSS hooks, and explicit cleanup |
...
const mounted = await mountPlayer(hostElement, puzzle, {
  ...
  css: `
    :scope { --accent: #87512b; --word: #f6e7d8; --font: Arial, sans-serif; }
...
Give the host an explicit height, such as `height: 80svh; min-height: 600px`. The returned `element` is a normal `.cross-player` DOM root that fills it. Host CSS can style its public hooks directly. Each instance owns its events, timers, IDs and scoped default styles; it leaves the host document title and routing alone. The default embedded view omits the site header; pass `chrome: 'full'` to include it. The puzzle controls remain available, including file opening in the save dialog.
...
For styling from your site stylesheet:

```css
.cross-player [data-cross-part="clue"] { border-radius: 0; }
```

Host stylesheet overrides affect the mounted view. Put styles in the `css` option when they should also travel with downloaded HTML; `:scope` refers to that player’s root.
