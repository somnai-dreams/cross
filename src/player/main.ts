...
const app = required('#app', HTMLDivElement)
if (embedded === null) throw new Error('Missing embedded puzzle')
const portable = parsePortable(embedded.textContent)
if (!portable.ok) { app.textContent = portable.error; throw new Error(portable.error) }
const configured = readConfiguration(document.querySelector('#crossword-config')?.textContent ?? null, portable.value.puzzle)
if (!configured.ok) { app.textContent = configured.error; throw new Error(configured.error) }
...
const initialPuzzle = library.find(entry => entry.puzzle.id === portable.value.puzzle.id)?.puzzle ?? portable.value.puzzle
...
// DOM nodes have page/puzzle lifetimes and stay out of serializable solving state.
const dom = {
  board: required('#board', HTMLDivElement), title: required('#puzzle-title', HTMLHeadingElement),
  boardWrap: required('.board-wrap', HTMLDivElement), dock: required('.mobile-dock', HTMLDivElement),
  boardColumn: required('.board-column', HTMLElement),
  gridSpace: required('.grid-space', HTMLDivElement), playLayout: required('.play-layout', HTMLDivElement),
  gridDetails: required('.grid-details', HTMLDivElement),
  clueLists: required('#clue-lists', HTMLElement), cluePanel: required('#clue-panel', HTMLElement),
  mobileHint: required('.mobile-hint', HTMLDivElement),
  zoom: required('#zoom-button', HTMLButtonElement),
  saveStatus: required('#save-status', HTMLSpanElement), mobileSaveStatus: required('#mobile-save-status', HTMLSpanElement),
  author: required('#puzzle-author', HTMLSpanElement), description: required('#puzzle-description', HTMLSpanElement),
  size: required('#puzzle-size', HTMLDivElement), timer: required('#timer', HTMLSpanElement),
  timerButton: required('#timer-button', HTMLButtonElement), timerIcon: required('#timer-icon', HTMLSpanElement),
  progress: required('#progress-label', HTMLSpanElement), pencil: required('#pencil', HTMLButtonElement),
  clueLabel: required('#clue-label', HTMLSpanElement), clueText: required('#clue-text', HTMLParagraphElement),
  clueLength: required('#clue-length', HTMLSpanElement), mobileLabel: required('#mobile-clue-label', HTMLSpanElement),
  mobileText: required('#mobile-clue-text', HTMLSpanElement), hintText: required('#hint-text', HTMLParagraphElement),
  hintButton: required('#hint-button', HTMLButtonElement), hintCard: required('#hint-card', HTMLDivElement),
  pauseCover: required('#pause-cover', HTMLDivElement), dialog: required('#dialog', HTMLDialogElement),
  dialogTitle: required('#dialog-title', HTMLHeadingElement), file: required('#puzzle-file', HTMLInputElement),
  across: required('#across-clues', HTMLDivElement), down: required('#down-clues', HTMLDivElement),
  toast: required('#toast', HTMLDivElement), announcement: required('#announcement', HTMLDivElement),
  completionDetail: required('#completion-detail', HTMLParagraphElement),
  copyLink: required('#copy-link', HTMLButtonElement), shareLink: required('#share-link', HTMLInputElement),
  libraryIntro: required('#library-intro', HTMLParagraphElement),
  ...
  switches: [...document.querySelectorAll<HTMLButtonElement>('.switch')],
}
...
function buildPuzzleDom(): void {
  ...
  for (let row = 0; row < puzzle.height; row++) {
    ...
    rowNode.className = 'grid-row'; rowNode.setAttribute('role', 'row')
    for (let col = 0; col < puzzle.width; col++) {
      ...
      number.className = 'cell-number'; number.setAttribute('aria-hidden', 'true')
      const start = starts.indexOf(index)
      number.textContent = start === -1 ? '' : String(puzzle.entries[start]!.number)
      const letter = document.createElement('span')
      letter.className = 'cell-letter'; letter.setAttribute('aria-hidden', 'true')
      if (cell.kind === 'letter' && cell.circled) { const circle = document.createElement('span'); circle.className = 'cell-circle'; node.append(circle) }
...
function render(now: number): void {
  ...
  document.title = `${puzzle.title} | ${brand}`
  ...
  dom.board.setAttribute('aria-activedescendant', `cell-${st.selection.cell}`)
  ...
  for (let i = 0; i < puzzle.cells.length; i++) {
    ...
    node.className = `cell${cell.kind === 'block' ? ' block' : ''}${active ? ' selected' : entry.cells.includes(i) ? ' word-selected' : ''}${fill.pencil ? ' pencilled' : ''}${fill.mark === 'incorrect' ? ' incorrect' : ''}${fill.mark === 'revealed' ? ' revealed' : ''}`
  ...
  for (let i = 0; i < puzzle.entries.length; i++) {
    ...
    dom.clues[i]!.className = `clue-row${current === entry ? ' active' : ''}${complete ? ' filled' : ''}`
  ...
  if (pendingDownload !== null) {
    ...
    void downloadPuzzle(puzzle, kind, snapshot)
  }
...
document.addEventListener('click', event => {
  ...
  if (commandNode !== null) { dispatch({ type: 'command', command: commandNode.dataset['command']! }); return }
  const cell = event.target.closest<HTMLElement>('[data-cell]')
  if (cell !== null) { dispatch({ type: 'cell', index: Number(cell.dataset['cell']) }); return }
  const entry = event.target.closest<HTMLElement>('[data-entry]')
  if (entry !== null) dispatch({ type: 'entry', index: Number(entry.dataset['entry']) })
})
...
dom.dialog.addEventListener('cancel', event => { event.preventDefault(); dispatch({ type: 'command', command: 'close' }) })
window.addEventListener('resize', () => dispatch({ type: 'layout' }))
...
window.addEventListener('popstate', event => dispatch({ type: 'navigate', search: location.search, historyState: event.state, source: 'history' }))
...
const keyboard = required('#keyboard', HTMLDivElement)
for (const [i, letters] of ['QWERTYUIOP', 'ASDFGHJKL', 'ZXCVBNM'].entries()) {
  const row = document.createElement('div'); row.className = 'key-row'
  if (i === 2) row.innerHTML = `<button class="key key-special" data-command="toggle-direction" aria-label="Switch direction">${icon('swap',20)}</button>`
  ...
  if (i === 2) row.insertAdjacentHTML('beforeend', `<button class="key key-special" data-command="erase" aria-label="Backspace">${icon('erase',22)}</button>`)
  keyboard.append(row)
}
...
const libraryList = required('#puzzle-library', HTMLDivElement)
for (const item of library) {
  const sample = item.puzzle
  const button = document.createElement('a'); button.className = 'library-item'; button.dataset['puzzleLink'] = ''
  button.href = `${location.pathname}?puzzle=${encodeURIComponent(item.slug)}`
  const preview = document.createElement('span'); preview.className = 'mini-preview'; preview.setAttribute('aria-hidden', 'true')
  preview.style.setProperty('--preview-columns', String(sample.width))
  preview.style.setProperty('--preview-rows', String(sample.height))
  for (const cell of sample.cells) { const square = document.createElement('i'); square.className = cell.kind === 'block' ? 'dark' : ''; preview.append(square) }
...
progress = restore(puzzle, portable.value.progress)
...
registerPlayerTools(request => new Promise(resolve => dispatch({ type: 'agent', request, resolve })))


async function downloadPuzzle(selected: Puzzle, kind: 'puz' | 'html' | 'progress', snapshot: Progress | null): Promise<void> {
  try {
    ...
    if (kind === 'puz') {
      ...
      if (embeddedScript === null || embeddedStyle === null) throw new Error('Missing embedded player assets')
