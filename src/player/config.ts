...
export function readConfiguration(text: string | null, initial: Puzzle): Result<PlayerConfiguration> {
  if (text === null) return { ok: true, value: { brand: 'Cross', storageKey: 'cross', homeUrl: null, mode: 'standalone', library: [] } }
  ...
  if (Object.keys(raw).some(key => !['version', 'brand', 'storageKey', 'mode', 'library', 'homeUrl'].includes(key))) return { ok: false, error: 'Unknown player configuration field.' }
  ...
  if (raw['mode'] === 'standalone') {
    ...
    return { ok: true, value: { brand: raw['brand'], storageKey: raw['storageKey'], homeUrl, mode: 'standalone', library: [] } }
  }
  ...
  return { ok: true, value: { brand: raw['brand'], storageKey: raw['storageKey'], homeUrl, mode: 'collection', library } }
}
