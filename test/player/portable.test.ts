...
test('import treats markup as data and rejects divergent HTML / binary puzzle copies', () => {
  ...
  expect(text).toContain('<title>&lt;/script&gt;&lt;script&gt;alert(1)&lt;/script&gt; ACROSS&amp;DOWN</title>')
  expect(text.slice(0, text.indexOf('<plaintext hidden'))).not.toContain('<script>alert(1)</script>')
